...
import { Clock, PlusCircle, Trash2, CalendarOff, AlertTriangle } from "lucide-react";
...
export default function BookingSettingsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [settRes, rulesRes, blockedRes] = await Promise.all([
      fetch("/api/admin/booking-settings?resource=settings"),
      ...
      >
        ...
        <div>
          ...
          <input
            ...
            min={0}
            max={60}
            defaultValue={settings?.booking_buffer_minutes ?? 0}
            onBlur={(e) =>
              saveSettings({ booking_buffer_minutes: parseInt(e.target.value, 10) || 0 })
            }
          ...
          {settings?.auto_confirm_bookings && (
            <div className="mt-3 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800 dark:border-amber-900/30 dark:bg-amber-900/10 dark:text-amber-300">
              ...
              <span>
                ...
                sinnvoll, wenn die Öffnungszeiten vollständig und korrekt
                gepflegt sind. Bei lückenhafter Konfiguration bleiben Anfragen
                auf &bdquo;Ausstehend&ldquo;.
              </span>
            </div>
          )}
        </div>
      ...
      >
              ...
              >
                <div>
                  ...
                  <span className="ml-2 text-xs text-slate-400">
                    ...
                    {rule.buffer_minutes > 0 && ` + ${rule.buffer_minutes} min Puffer`}
                  </span>
                </div>
                <button
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            <div>
              <label className="block text-xs text-slate-500 mb-1">Wochentag</label>
              <select
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setNewRule({ ...newRule, slot_minutes: parseInt(e.target.value, 10) })
                }
                className="w-full rounded-lg border px-2 py-1.5 text-sm"
                style={{
                  borderColor: "var(--color-border)",
