...
export default function AdminBookingRequestsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch(`/api/admin/booking-requests?filter=${filter}`, { cache: "no-store" });
    if (res.ok) {
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <span className={`font-semibold text-sm ${getBookingStatusColor(item.status)}`}>
                  ...
                  <div className="mt-2 space-y-0.5 text-sm text-slate-700 dark:text-slate-300">
                    ...
                    <p><strong>E-Mail:</strong> {item.patient_email}</p>
                    {item.patient_phone && (
                      <p><strong>Telefon:</strong> {item.patient_phone}</p>
                    )}
                    <p><strong>Zeitraum:</strong> {item.preferred_time}</p>
                    {item.note && (
                ...
                {item.status === "pending_confirmation" && (
                  <div className="flex shrink-0 flex-col gap-2">
                    ...
                    >
                      <MessageSquare className="h-3.5 w-3.5 text-slate-400" /> Notiz
                    </button>
                  </div>
                )}
              ...
              {noteId === item.id && (
                <div className="mt-3 space-y-2">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => doAction(item.id, "add_note", noteText ? { internal_note: noteText } : {})}
